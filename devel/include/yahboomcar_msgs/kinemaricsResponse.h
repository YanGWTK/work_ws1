  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::yahboomcar_msgs::kinemaricsResponse_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.target_joints.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.target_joints[i]);
    }
    if (v.target_joints.empty() || true)
      s << "]";
    ...
    for (size_t i = 0; i < v.target_pos.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.target_pos[i]);
    }
    if (v.target_pos.empty() || true)
      s << "]";
  }
