  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::yahboomcar_msgs::kinemaricsRequest_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.src_pos.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.src_pos[i]);
    }
    if (v.src_pos.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.src_joints.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.src_joints[i]);
    }
    if (v.src_joints.empty() || true)
      s << "]";
  }
